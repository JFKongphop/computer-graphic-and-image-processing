...
- Pixel-perfect indexing and alpha blending formulas
- Circle and line drawing algorithms with mathematical proofs
- Xiaolin Wu's anti-aliasing algorithm explained step-by-step
- GPS coordinate transformation to image pixels

Each topic includes: the formula, detailed examples with numbers, code implementation, and visual diagrams.
...
- **Basic coordinate systems**: (x, y) positions on a grid
- **Basic algebra**: Variables, equations, substitution
- **Color models**: RGB/BGR (Red, Green, Blue channels)
- **Programming concepts**: Arrays, loops, functions

**Not required:**
- Calculus or advanced mathematics
- Specific programming language expertise
- Prior graphics programming experience

**Mathematical notation:**
- $x^2$ means "x squared"
- $\sqrt{x}$ means "square root of x"
- $\lfloor x \rfloor$ means "floor of x" (round down)
- $\Delta x$ means "change in x" ($x_1 - x_0$)

---

## Quick Example: Drawing a Route

Before diving into details, here's what these functions accomplish together:

**Input:** GPS route data from cycling activity
```
Points: [(37.7749, -122.4194), (37.7750, -122.4180), ...]
```

**Processing:**
1. Normalize GPS to 0.0-1.0 range
2. Map to image pixel coordinates
3. Draw thick anti-aliased lines between points
4. Draw circles at start/end markers

**Output:** Smooth route visualization on image

This document explains the mathematics behind each step.

---

## Function Architecture

The FastImage system has three layers:

### **Layer 1: Data Structures**
- `from_mat()` - OpenCV Mat → Flat array
- `to_mat()` - Flat array → OpenCV Mat

### **Layer 2: Pixel Operations**
- `put_pixel_bgr()` - Set single pixel with alpha blending
  - Uses: 2D→1D indexing, color blending

### **Layer 3: Shape Drawing**
- `draw_circle()` - Draw filled circles
  - Uses: Circle equation, put_pixel_bgr()
  
- `draw_line_aa_single()` - Draw thin anti-aliased line
  - Uses: Wu's algorithm, put_pixel_bgr()
  
- `draw_line_aa()` - Draw thick anti-aliased line
  - Uses: Perpendicular vectors, draw_line_aa_single()

### **Layer 4: High-Level Utilities**
- `load_and_resize_image()` - Image scaling with aspect ratio
- `read_route_from_fit()` - GPS → pixel coordinate transformation

**Reading order:** Bottom-up (Layer 1 → Layer 4) or Top-down (Layer 4 → Layer 1)

This document follows bottom-up order: foundations first, then building complexity.
...
## 1. from_mat() - Matrix to Buffer Conversion
...
## 3. put_pixel_bgr() - Pixel Indexing & Alpha Blending
...
## 5. draw_line_aa_single() - Wu's Algorithm
...
## 6. draw_line_aa() - Thick Line with Perpendicular Vectors
...
## 7. load_and_resize_image() - Image Scaling
...
## 8. read_route_from_fit() - GPS Coordinate Normalization
...
| Operation | Formula | Plain English |
|-----------|---------|---------------|
| **Pixel Index** | $\text{idx} = (y \times \text{width} + x) \times 3$ | Skip rows, move right, times 3 |
| **Alpha Blend** | $C = C_{\text{old}} + (C_{\text{new}} - C_{\text{old}}) \times \alpha$ | Blend based on transparency |
| **Circle Test** | $x^2 + y^2 \leq r^2$ | Distance from center ≤ radius |
| **Line Slope** | $m = \Delta y / \Delta x$ | Rise over run |
| **Vector Length** | $\|\|\vec{v}\|\| = \sqrt{x^2 + y^2}$ | Pythagorean theorem |
| **Unit Vector** | $\hat{v} = \vec{v} / \|\|\vec{v}\|\|$ | Normalize to length 1 |
| **Perpendicular** | $(x, y) \rightarrow (-y, x)$ | Rotate 90° counterclockwise |
| **Normalize GPS** | $(v - \min) / (\max - \min)$ | Convert to 0-1 range |
| **Scale Image** | $\text{new} = \text{old} \times \text{scale}$ | Multiply both dimensions |
...
### **10. Perpendicular Unit Vector**
$$\vec{p}_{\perp} = \left(\frac{-dy}{\text{length}}, \frac{dx}{\text{length}}\right)$$

### **11. Parallel Line Offset**
$$\text{offset} = i - \frac{\text{thickness}}{2} + 0.5$$
...
### Academic Papers
- **Xiaolin Wu's Line Algorithm**: Wu, Xiaolin (1991). "An efficient antialiasing technique". *Computer Graphics*, 25(4): 143–152
- **Alpha Compositing**: Porter, Thomas; Tom Duff (1984). "Compositing Digital Images". *SIGGRAPH '84 Proceedings*
- **Bresenham's Algorithm**: Bresenham, J. E. (1965). "Algorithm for computer control of a digital plotter". *IBM Systems Journal*, 4(1): 25–30
...
- Wikipedia: [Circle equation](https://en.wikipedia.org/wiki/Circle#Equations)
- Wikipedia: [Feature scaling](https://en.wikipedia.org/wiki/Feature_scaling)

### Books
- **Computer Graphics: Principles and Practice (3rd Edition)** - Hughes, van Dam, et al.
- **Real-Time Rendering (4th Edition)** - Akenine-Möller, Haines, Hoffman
- **Fundamentals of Computer Graphics (5th Edition)** - Marschner & Shirley

### Technical Documentation
- [OpenCV Documentation](https://docs.opencv.org/)
- [FIT SDK Documentation](https://developer.garmin.com/fit/protocol/) - Garmin
